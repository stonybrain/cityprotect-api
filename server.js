...
const fetchJSON = async (url, opts = {}) => {
  ...
  clearTimeout(t);
  // if EP ever returns non-JSON on errors, fall back to text
  const ct = r.headers.get("content-type") || "";
  if (ct.includes("application/json")) return r.json();
  const txt = await r.text();
  try { return JSON.parse(txt); } catch { return { _raw: txt, status: r.status }; }
};

// Simple zone classifier for Redding
...
/* ---------- Core fetcher you can reuse for any hours window ---------- */
async function fetchWindow(hours = 72) {
  const clamped = Math.max(1, Math.min(72, Number(hours) || 72));
  const now = new Date();
  const from = new Date(now.getTime() - clamped * 60 * 60 * 1000);
...
/* ---------- Old fixed 72h endpoint (kept for compatibility) ---------- */
app.get("/api/redding-72h", async (_req, res) => {
  try {
    const data = await fetchWindow(72);
    res.set("Cache-Control", "no-store");
    res.json(data);
  } catch (e) {
...
/* ---------- New dynamic endpoint: /api/redding?hours=1..72 ---------- */
app.get("/api/redding", async (req, res) => {
  try {
    const data = await fetchWindow(req.query.hours);
    res.set("Cache-Control", "no-store");
    res.json(data);
  } catch (e) {
...
app.get("/api/push/discord/summary", async (req, res) => {
  try {
    ...
    const data = await fetchWindow(hours);

    const total = data.total;
    const header = `**Redding Crime — Summary (${hours}h)**\nTotal incidents: **${total}**`;
    const lines = data.incidents.slice(0, 10).map(i =>
      `• ${i.type} — ${i.zone} (${i.lat?.toFixed(4)}, ${i.lon?.toFixed(4)})`
    );
...
// Convenience alias: /api/push/discord -> same as summary default 6h
app.get("/api/push/discord", async (req, res) => {
  ...
  try {
    ...
    console.error("DISCORD alias error:", e);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
app.listen(PORT, () => console.log("cityprotect-api on :" + PORT));// server.js
import express from "express";
import fetch from "node-fetch";
